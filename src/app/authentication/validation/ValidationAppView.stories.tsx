...
export const Default: StoryObj<typeof ValidationAppView> = {
  render: (args) => (
    <Routes>
      <Route path={URLs.registration_validation} element={<ValidationAppView {...args}/>}/>
      <Route path={"/"} element={<Navigate to={URLs.registration_validation.replace(":token", "some-validation-token")}/>}/>
    </Routes>
  )
}